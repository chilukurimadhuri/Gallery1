...
function effects(){
    ...
    rotateX(${rotXVal}deg) rotateY(${rotYVal}deg) scale(${scaleVal})
    scaleX(${scaleXVal}) scaleY(${scaleYVal}) skew(${skewVal}deg)
    skewX(${skewXVal}deg) skewY(${skewYVal}deg) `
    ...
    hue-rotate(${hueVal}deg) invert(${invVal}%) opacity(${opaVal}%) 
    saturate(${satVal}%) sepia(${sepVal}%)  `
}
